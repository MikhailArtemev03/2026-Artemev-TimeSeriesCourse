...
class TimeSeriesHierarchicalClustering:
    ...
    def fit(self, distance_matrix: np.ndarray) -> Self:
        ...
        ----------
        distance_matrix:
            distance matrix between instances of dataset
            with shape (ts_number, ts_number)
    ...
    def _draw_timeseries_allclust(
        ...
        ----------
        dx:
            timeseries data with column "y"
            indicating cluster number
        ...
        for cnt in range(len(leaves)):
            ...
            plt.subplot(
                gs[
                    cnt:cnt + 1,
                    max_cluster - ts_hspace:max_cluster
                ]
            )
            ...
            # get leafnode name,
            # which corresponds to original data index
            leafnode = leaves[cnt]
    ...
    def plot_dendrogram(
        ...
        with timeseries graphs for all clusters.

        Parameters
        ----------
        df:
            dataframe with each row being
            the time window of readings
        ...
        ddata = dendrogram(
            self.linkage_matrix,
            orientation="left",
            color_threshold=sorted(
                self.model.distances_
            )[-2],
            show_leaf_counts=True
        )

        self._draw_timeseries_allclust(
            df,
            labels,
            ddata["leaves"],
            gs,
